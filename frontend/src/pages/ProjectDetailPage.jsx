      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="card space-y-4">
          ...
          <div className="space-y-2.5 text-sm">
            ...
            <div className="flex justify-between pt-2" style={{ borderTop: '1px solid #2E2E2E' }}>
        ...
        {salary && (
          <div className="card md:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ['Бюджет', formatMoney(salary.budget), 'text-white', '#272727'],
                [`Налог ${salary.taxRate}%`, `−${formatMoney(salary.tax)}`, 'text-yellow-400', 'rgba(245,158,11,0.1)'],
                ['К распределению', formatMoney(salary.distributable), 'text-primary-400', 'rgba(233,84,32,0.1)'],
                ['Профит студии', formatMoney(salary.companyProfit), 'text-green-400', 'rgba(16,185,129,0.1)'],
              ].map(([l, v, c, bg]) => (
                <div key={l} className="rounded-xl p-3 text-center" style={{ background: bg, border: '1px solid #2E2E2E' }}>
            ...
            {salary.breakdown.length > 0 ? (
              <div className="space-y-2">
                {salary.breakdown.map((b, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: '#1A1A1A', border: '1px solid #2E2E2E' }}>
                    <img
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(b.employee.name)}&backgroundColor=e95420&fontFamily=Ubuntu&fontSize=38`}
